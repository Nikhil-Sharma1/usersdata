...
router.post('/adduser', async (req, res) => {
  const { fname, lname, phone, email, password, course, country, date } = req.body;
  if (!fname || !lname || !course || !country || country.length === 0 || !phone || !email || !password) {
  ...
  try {
    ...
    const user = new User({ fname, lname, phone, email, password, course, country, date });
...
router.post('/updateuser', authenticate, async (req, res) => {
  const { fname, lname, phone, course, country, date } = req.body;
  if (!fname || !lname || !course || !country || country.length === 0 || !phone) {
    return res.status(401).json({ error: "Plz fill all the fields" });
  }
  const phoneRegex = /^([+]?[\s0-9]+)?(\d{3}|[(]?[0-9]+[)])?([-]?[\s]?[0-9])+$/i;
  if ((phoneRegex.test(phone) === false) || phone.length !== 10) {
